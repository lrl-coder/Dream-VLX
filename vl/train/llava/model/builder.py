    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                cfg_pretrained = LlavaDreamConfig.from_pretrained(model_path, trust_remote_code=True)
                model = LlavaDream.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
                # model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
            
            elif "qwen" in model_name.lower() or "quyen" in model_name.lower():
                # hard coded
                # model_path = "Dream-org/Dream-v0-Instruct-7B"
                # print("Load Qwen AR model but initialized with Dream")
