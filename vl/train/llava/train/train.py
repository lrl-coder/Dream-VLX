...
class ModelArguments:
    ...
    vision_custom_tunable_parts: Optional[bool] = field(default=False, metadata={"help": "Call .custom_tunable_parts() to decided which params are learnable in vision_tower"})
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    assert len(im_start) == 1 and len(im_end) == 1
    im_start, im_end = im_start[0], im_end[0]
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            # conv = [{"role" : role, "content" : content.replace("<image>", "<|vision_start|><image><|vision_end|>")}]
        ...
        for idx, encode_id in enumerate(input_id):
            if encode_id in unmask_tokens_idx: # not sure why have loss on these query-part tokens
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            # if image.size()[0]*image.size()[1] > processor.max_pixels:
            #     print(f"resize the images pixels {image.size()[0]*image.size()[1]} ({image.size()}) to {processor.max_pixels}")
        else:
            image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1 and self.data_args.image_aspect_ratio != 'native_anyres':
                    ...
                    for idx in sampled_indices:
                        ...
                        try:
                            with Image.open(frame_path) as img:
                                ...
                                video.append(frame) # list of PIL.Image
                        except IOError:
                            print(f"Failed to read frame at path: {frame_path}")
                else:
                    video, video_time, frame_time, num_frames_to_sample = process_video_with_decord(video_file, self.data_args)
                    if num_frames_to_sample % 2 != 0:
                        # old qwen vl processor not support odd video frames
                        video = np.concatenate([video, video[-1:]], axis=0)
                        video = video.transpose(0, 3, 1, 2)
                        num_frames_to_sample += 1

                processor = self.data_args.image_processor
                if self.data_args.image_aspect_ratio == "native_anyres":
                    res = processor.preprocess(images=None, videos=video, return_tensors="pt")
                    image = res['pixel_values_videos']
                    image_grid_thw = res['video_grid_thw']  # [[ 1, grid_H, grid_W ]]
                    image_size = image_grid_thw[0]
                    image_size[0] *= image_grid_thw.shape[0] # [ N_frames, grid_H, grid_W ]
                else:
                    res = processor.preprocess(video, return_tensors="pt")
                    image = res['pixel_values']
                    image_size = video[0].size ## todo
                if self.data_args.add_time_instruction:
                    time_instruciton = f"The video lasts for {video_time:.2f} seconds, and {num_frames_to_sample} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following questions related to this video."
                    sources[0]["conversations"][0]["value"] = f'{DEFAULT_IMAGE_TOKEN}\n{time_instruciton}\n{sources[0]["conversations"][0]["value"].replace(DEFAULT_IMAGE_TOKEN, "")}'
                image = [(image, image_size, "video")]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                # print(sources)
            except Exception as e:
                print(f"Error: {e}")
        ...
        if "image" in self.list_data_dict[i]:
            ...
            # print(f"text-only data, set image to zero tensors")
            if self.data_args.image_aspect_ratio == "native_anyres":  # 448
                data_dict['image'] = [[torch.zeros(1024, 1176), torch.tensor([1, 32, 32]), "text"]]
                data_dict['image_sizes'] = torch.tensor([1, 32, 32])  # [ t, image_h, image_wz ]
            else:
        ...
        # print("get item done")
        try:
            a = [im[1] for im in data_dict['image']]
        except:
            print(f"unknown error in data {data_dict['image']}, skipping data...")
            return self._get_item(i + 1)
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch = dict(input_ids=input_ids, labels=labels.long() if labels.dtype == torch.int32 else labels, attention_mask=input_ids.new_ones(input_ids.shape).bool())
        ...
        if "image" in instances[0]:
            ...
            if self.image_aspect_ratio == "native_anyres":
                ...
                batch['image_sizes'] = torch.stack(batch["image_sizes"])
            else:
                if all(x is not None and x.shape == images[0].shape for x in images):
                    batch['images'] = torch.stack(images)
                else:
        ...
        # print("batchfy items done")
        return batch

def show_statistics(train_dataset, inds):
    length = []
    for ind in inds:
        item = train_dataset[int(ind)]
        #### for qwenvit
        # l = 0
        ...
        # length.append(l)
        #### for siglip
        length.append(len(item['image'])*729)
    length = np.array(length)
    max_value = np.max(length)
    mean_value = np.mean(length)
    min_value = np.min(length)
    percentile_25 = np.percentile(length, 25)
    percentile_75 = np.percentile(length, 75)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    # np.random.seed(0);show_statistics(train_dataset, np.random.randint(0, len(train_dataset), size=100))
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer, image_aspect_ratio=data_args.image_aspect_ratio)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            model = LlavaDream.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_vision_tower" in tunable_parts:
                if model_args.vision_custom_tunable_parts:
                    rank0_print(f"Using custom_tunable_parts")
                    model.get_model().vision_tower.custom_tunable_parts()
                else:
