...
CHECKPOINT= # an intermediate training ckpt
...
for i in "${!TASKS_ARRAY[@]}"; do
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --num_processes=8 --main_process_port 29591 -m lmms_eval --model llava_onevision \
        --model_args pretrained=${CHECKPOINT},conv_template=qwen_2,model_name=llava_dream \
        --tasks ${TASKS_ARRAY[$i]} \
