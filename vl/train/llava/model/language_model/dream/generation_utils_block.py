...
class DreamGenerationMixin:
    ...
    def _expand_inputs_for_generation(
        ...
        if inputs_embeds is not None:
            inputs_embeds = inputs_embeds.repeat_interleave(expand_size, dim=0)
        return input_ids, inputs_embeds, attention_mask
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        if inputs_embeds is not None:
            ...
            input_ids = torch.full((bsz, seqlen), -100, device=inputs_embeds.device)
        ...
        input_ids, inputs_embeds, attention_mask = self._expand_inputs_for_generation(
    ...
    def _sample(
        ...
        inputs_embeds: torch.Floatensor,
        ...
        x = F.pad(input_ids, (0, max_length - prefix_length), value=mask_token_id)
        inputs_embeds = F.pad(inputs_embeds, (0, 0, 0, max_length - prefix_length), value=0.)
        gen_length = max_length - prefix_length
        ...
        for num_block in range(num_blocks):
            ...
            if inputs_embeds is not None:
                inputs_embeds[:, prefix_length:] = self.get_input_embeddings()(x[:, prefix_length:])

            # update cache
            model_output = self(inputs_embeds=inputs_embeds, attention_mask=attention_mask, position_ids=tok_idx, use_cache=True)
